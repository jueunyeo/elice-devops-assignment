...
본 레포지토리는 **인프라 관리 및 GitOps 전용 레포지토리**입니다.  
파이프라인을 인프라와 서비스로 분리해 안정성과 배포 민첩성을 동시에 확보합니다.

### **A. Infrastructure Pipeline (Atlantis + Terraform)**

```text
[Infra PR: aws/terraform | on-prem/terraform]
            |
            v
       [Atlantis Plan]
            |
   (PR Approved Required)
            |
            v
      [Atlantis Apply]
            |
            v
[AWS/On-prem Infra Provisioning]
```

- **실행 주체**: Atlantis (EKS 내 Pod)
- **대상 범위**: `aws/terraform/`**, `on-prem/terraform/**`
- **승인 게이트**: `apply_requirements: [approved]`
- **보안 모델**: Atlantis IRSA Role -> 각 계정 Assume Role (`OrganizationAccountAccessRole`)

### **B. Service Pipeline (External CI + ArgoCD)**

```text
[App Repo CI: Build/Test/Image Push]
            |
            v
[External GHA updates values-{env}.yaml in this repo]
            |
            v
   [ArgoCD App-of-Apps detects Git change]
            |
            v
 [Auto Sync (prune + selfHeal) to clusters]
```

- **실행 주체**
  - 이미지 빌드/테스트: 외부 애플리케이션 레포 CI
  - 배포 타겟 업데이트: 이 레포의 `values-{env}.yaml`
  - 배포 적용: ArgoCD (클러스터 내부)
- **배포 타겟 경로**
  - `aws/kubernetes/services/<service>/values-{env}.yaml`
  - `on-prem/kubernetes/services/<service>/values-{env}.yaml`
- **ArgoCD 동작**
  - Root App (`main-app.yaml`)이 child app 및 values 변경을 지속 감시
  - `automated: { prune: true, selfHeal: true }` 기반 자동 동기화

### **Multi-repo Deployment Guide**

개발자는 이 레포지토리에 직접 커밋하지 않고, 외부 애플리케이션 레포 CI에서 `repository_dispatch` API로만 배포 신호를 전달합니다.

```mermaid
flowchart LR
  A[App Repo CI] --> B[Dispatch Signal]
  B --> C[service-deploy-handler.yml in Infra Repo]
  C --> D[Update values-{env}.yaml]
  D --> E[ArgoCD]
  E --> F[Cluster Sync]
```
...
- **Dispatch 수신 워크플로우**: `.github/workflows/service-deploy-handler.yml`
- **핵심 검증**:
  - 허용된 서비스(`api-gateway`, `portal-web`, `user-api`, `media-manager`)만 처리
  - 허용된 환경(`dev`, `stage`, `prod`)만 처리
  - 대상 파일 존재 여부 확인 후 `.image.tag` 업데이트
- **커밋 메시지 규칙**:
  - `[Deploy] <service> to <env> with tag <tag> [skip ci]`

### **Helm Chart Standardization**

- **elice-common**: 모든 마이크로서비스가 공통으로 사용하는 Standard Helm Chart입니다.
...
#### **1) EKS Control Plane Endpoint Isolation**

- EKS API Endpoint는 `endpoint_public_access=true`를 유지하되, `public_access_cidrs`를 통해 허용된 CIDR에서만 접근 가능하도록 제한했습니다.
- 동시에 `endpoint_private_access=true`를 적용해 내부 노드/워크로드와의 통신 효율 및 보안성을 강화했습니다.
- 환경별로 `eks_public_access_cidrs` 변수를 통해 접근 대역을 분리 관리합니다.

#### **2) Data Encryption & Deletion Safety**

- **RDS**
  - `storage_encrypted=true` 적용
  - AWS 관리형 기본키 대신 서비스 전용 `aws_kms_key` 생성 후 연결
  - `deletion_protection=true`로 실수 삭제 방지
- **S3**
  - SSE를 KMS 기반(`aws:kms`)으로 전환
  - 버킷별 전용 `aws_kms_key`와 alias를 생성해 연결
  - `bucket_key_enabled=true`로 KMS 비용/성능 효율 개선

#### **3) Zero Trust Network Policy**

- `aws/kubernetes/services/`* 및 `on-prem/kubernetes/services/*`에 서비스별 `network-policy.yaml`을 추가했습니다.
- 기본 정책은 **Default Deny All**이며, 필요한 통신만 Label 기반으로 허용합니다.
- 핵심 허용 경로:
  - Ingress Controller -> `api-gateway`
  - `api-gateway` -> `user-api`
  - `user-api` -> `postgres`(TCP/5432)
  - DNS 질의(UDP/TCP 53)

#### **4) Pod Runtime Security (PSS Restricted 방향)**

- 공통 Helm 차트(`elice-common`)의 Deployment 템플릿 기본 보안 컨텍스트를 강화했습니다.
  - `runAsNonRoot: true`
  - `allowPrivilegeEscalation: false`
  - `readOnlyRootFilesystem: true`
  - `capabilities.drop: [ALL]`
- 이를 통해 기본 런타임 공격 표면(권한 상승, 쓰기 가능한 루트FS)을 축소합니다.

#### **5) Image Security & Secret Hygiene**

- ECR Repository 생성 시 `scan_on_push=true`를 활성화해 이미지 푸시 단계에서 취약점 스캔을 수행합니다.
- Harbor values 파일에 평문 비밀번호를 두지 않도록 정책을 명시하고, External Secrets 기반 주입을 가이드했습니다.
...
1. **Terraform Backend 초기화 (AWS Management Account)**
  - 경로: `aws/terraform/bootstrap/state-backend/`
  - 동작: 관리자 권한으로 위 경로의 코드를 1회 수동 배포하여 리소스를 생성합니다.
  - 목적: 중앙 S3 State Bucket + DynamoDB Lock Table 생성
2. **Atlantis 서버 배포 (AWS Management Account / EKS)**
  - 경로: `aws/terraform/bootstrap/atlantis/`
  - 동작: 관리자 권한으로 위 경로의 코드를 1회 수동 배포하여 리소스를 생성합니다.
  - 구성:
    - Helm 기반 Atlantis 배포
    - IRSA Role 생성 및 Atlantis ServiceAccount 연결
    - 하위 계정(`dev`, `stage`, `prod`) `OrganizationAccountAccessRole` Assume 정책 부여
    - AWS Load Balancer Controller 기반 ALB Ingress (HTTPS 전용)
    - AWS Secrets Manager에서 `github_token`, `webhook_secret` 주입
3. **Harbor 서버 배포 (On-prem Kubernetes)**
  - 경로: `on-prem/terraform/bootstrap/harbor/`
  - 동작: 관리자 권한으로 위 경로의 코드를 1회 수동 배포하여 리소스를 생성합니다.
  - 구성:
    - Helm 기반 Harbor 배포
    - `on-prem/terraform/environments/{env}/addons/values-harbor.yaml`을 `templatefile`로 로드
    - 기존 StorageClass를 사용하는 PVC 설정 주입
    - NGINX Ingress 또는 MetalLB/NodePort 모드 선택 가능

### **Step 2: External Credential Integration**

각 파이프라인 실행 주체별로 권한을 분리 연동합니다.

- **Atlantis (Infra)**:
  - EKS ServiceAccount + IRSA Role 연동
  - 하위 계정 `OrganizationAccountAccessRole` Assume 권한
- **External App CI (Service)**:
  - 이 레포지토리 push 권한이 있는 GitHub Token/App
  - 변경 대상은 `values-{env}.yaml` 파일의 `image.tag` 필드로 제한

### **Step 3: Provisioning via Pull Request (CI)**

모든 인프라 수정은 Atlantis 기반 PR 워크플로우로 검증/반영됩니다.

1. 브랜치에서 인프라 코드를 수정 후 PR을 생성합니다.
2. Atlantis가 변경된 Terraform 프로젝트 기준으로 `plan`을 수행합니다.
3. PR 승인 후에만 `atlantis apply`가 허용됩니다(`apply_requirements: [approved]`).
